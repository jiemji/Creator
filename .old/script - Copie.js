document.addEventListener('DOMContentLoaded', () => {
    ...
    let coreSkillPointsRemaining = 32;
    ...
    function randomizeHistory() {
		...
		const misfortune = getRandomItem(gameData.lose).malheur;
    ...
    function calculatePickupSkillPoints() {
        if (characterAttributes['Intelligence'] && characterAttributes['Réflexes']) {
            pickupSkillPointsRemaining = characterAttributes['Intelligence'] + characterAttributes['Réflexes'];
            
            // Recalculer les points dépensés car le total a peut-être changé
            for(const skillName in characterSkills) {
                const isCore = gameData.coreskill.some(cs => cs.Classe === domElements.identity.lifepath.value && cs.Skill === skillName);
                if (!isCore) {
                    pickupSkillPointsRemaining -= characterSkills[skillName];
                }
            }
        }
    ...
    function changeSkill(skillName, isCore, action) {
        if (action === 'increase' && characterSkills[skillName] < MAX_SKILL) {
    ...
    function updateSkillButtons() {
        document.querySelectorAll('.skill-controls .quantity-btn').forEach(btn => {
            ...
            if (action === 'increase') {
                if (characterSkills[skillName] >= MAX_SKILL) {
                    btn.disabled = true;
                } else if (isCore) {
...
${skillsText || 'Aucune compétence acquise.'}
    ...
    async function initializeApp() {
        ...
        try {
            const filePaths = ['vetement.csv', 'coiffure.csv', 'accessoire.csv', 'origine.csv', 'childhood.csv', 'ethnie.csv', 'destin.csv', 'lose.csv', 'stat.csv', 'competences.csv', 'coreskill.csv'];
            ...
            const [vetement, coiffure, accessory, origine, childhood, ethnie, destin, lose, stats, competences, coreskill] = results;
            Object.assign(gameData, { vetement, coiffure, accessoire: accessory, origine, childhood, ethnie, destin, lose, stats, competences, coreskill });
